...
"$clang" --target=x86_64-elf -std=c11 -ffreestanding -fno-stack-protector \
    ...
    -Wall -Wextra -Werror -Wpedantic -c apps/edit/edit.c -o "$temporary.o"
"$clang" --target=x86_64-elf -c apps/edit/start.S -o "$temporary.start.o"
"$ld" -pie -nostdlib -T apps/edit/linker.ld -e _start \
...
if [ "$code_size" -gt 8176 ]; then
    echo "editor exceeds two AWP code pages" >&2
    exit 1
fi
