...
"$clang" --target=x86_64-elf -std=c11 -ffreestanding -fno-stack-protector \
    -fno-stack-check -fpie -fno-plt -mno-red-zone -mcmodel=small \
    -Wall -Wextra -Werror -Wpedantic -c apps/calc/calc.c -o "$temporary.o"
"$clang" --target=x86_64-elf -c apps/calc/start.S -o "$temporary.start.o"
"$ld" -pie -nostdlib -T apps/calc/linker.ld -e _start -o "$temporary.elf" "$temporary.start.o" "$temporary.o"
"$objcopy" -O binary "$temporary.elf" "$temporary.bin"

code_size=$(wc -c < "$temporary.bin" | tr -d ' ')
if [ "$code_size" -ge 2048 ]; then
    echo "calculator exceeds one AWP code page" >&2
    exit 1
fi
