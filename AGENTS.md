...
- Treat AWP syscalls `22` and `23` as the secure-runtime boundary in ADR-0065.
  `random_fill` returns at most 256 CPU-generated bytes and fails closed;
  `realtime` returns CMOS-derived Unix UTC seconds. TLS, X.509, trust roots,
  DNS, HTTP, and future SSH protocol state remain in user space. Future SSH
  may reuse the secure-runtime contracts and reviewed cryptographic primitives,
  but not TLS records, X.509, or Web trust policy.
- Keep the expanded AWP allocation fixed and bounded: 48 code pages and 24
  stack pages per slot. Seeded executable images may be read up to 192 KiB,
  but mutable ARFS writes and whole-text-file consumers retain their existing
  8192-byte and narrower consumer bounds.
